...
def login_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            user = form.get_user()
            login(request, user)
            # Redirect the user to the base view after successful login
...
def signup_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            user = form.save()
            login(request, user)
            # Redirect the user to the base view after successful signup
            return redirect('base')
    else:
...
def classification(request):
    ...
    if request.session:
        ...
        # Assign a default value to nameMethod
        nameMethod = ''

        if request.session:
            method = request.session.get('method','salam')
...
def clustering(request):
    ...
    if len(features) >= 2:
            ...
            # Handle the case where 'features' list does not have enough elements
            # For example, you could set default values or redirect the user
            # In this example, I'm setting default values to avoid the IndexError
            nilai_x = 'Default X Value'
            nilai_y = 'Default Y Value'
